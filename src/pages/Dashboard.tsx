...
export function Dashboard() {
  ...
  // Modal de detalle de oferta/combo (reutilizable en ambas vistas)
      ...
      {comboDetail && (
        ...
        >
          <motion.div
            ...
            animate={{ scale: 1, y: 0 }}
            exit={{ scale: 0.9, y: 15 }}
            className="bg-white border border-slate-200 rounded-[2rem] w-full max-w-sm shadow-2xl relative overflow-hidden max-h-[85vh] overflow-y-auto"
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={() => setComboDetail(null)}
              className="absolute top-4 right-4 text-slate-400 hover:text-ink transition-colors bg-white/80 p-2 rounded-full cursor-pointer border-none z-10"
            >
              <X size={14} />
            </button>
            ...
            <div className="p-6">
              <h3 className="text-base font-black uppercase !text-slate-900 tracking-tight">{comboDetail?.title}</h3>

              <div className="mt-3 space-y-3">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Descripción</p>
                  <p className="text-xs text-slate-600 leading-relaxed">{comboDetail?.description || 'Disfruta de nuestros menús premium precargados.'}</p>
                </div>
    ...
    return (
      ...
      >
        ...
        <div className="flex justify-between items-center px-1">
          <div>
            <h1 className="text-2xl font-black uppercase tracking-tight leading-none !text-slate-900" style={{ fontFamily: "'Oswald', sans-serif" }}>Hola, {profile.full_name?.split(' ')[0]}</h1>
          </div>
        </div>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <div className="relative flex flex-col items-center filter drop-shadow-[0_3px_6px_rgba(217,0,21,0.5)]">
                    ...
                    <div className="w-5.5 h-9 bg-white rounded-b-sm border border-slate-200 relative overflow-hidden flex items-center justify-center -mt-0.5" style={{ clipPath: 'polygon(3% 0%, 97% 0%, 82% 100%, 18% 100%)' }}>
                      <span className="text-[7px] font-black tracking-[0.15em] text-love select-none" style={{ fontFamily: "'Oswald', sans-serif", transform: 'rotate(-90deg)', whiteSpace: 'nowrap' }}>
                        CRAFT
                      </span>
                    </div>
                  </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="text-sm font-black uppercase tracking-wider !text-slate-900" style={{ fontFamily: designConfig?.fontHeadings || 'inherit' }}>Comprá Ofertas</h3>
            </div>
        ...
        {/* Modal de detalle de combo (reutilizable) */}
        {comboDetailModal}
      </motion.div>
    );
  }
